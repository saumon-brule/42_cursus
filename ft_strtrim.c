...
/*   Updated: 2024/11/13 18:32:06 by ebini            ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "./libft.h"

static char	*ft_strrchr_len(const char *s, int c, size_t len)
{
	while (--len + 1)
		if (s[len] == (char)c)
			return ((char *)s + len);
...
char	*ft_strtrim(char const *s1, char const *set)
{
	...
	if (result_len)
		while (ft_strrchr_len(set, s1[result_len - 1], ft_strlen(set)))
			result_len--;
